...
{
    public partial class AntrenörSil : Form
    {
        public AntrenörSil()
        {
            InitializeComponent();
        ...
        private void btnSil_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Bu verileriniz silinecektir. Kabul ediyor musunuz?", "Veriyi Sil", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                using (NpgsqlConnection con = new NpgsqlConnection(connString))
                {
                    ...
                    {
                        con.Open();
                        string deleteQuery = "DELETE FROM new_coach WHERE coach_id = @coach_id";
                        using (NpgsqlCommand cmd = new NpgsqlCommand(deleteQuery, con))
                        {
                            cmd.Parameters.AddWithValue("coach_id", int.Parse(textBox1.Text));
                            cmd.ExecuteNonQuery();
                        }
        ...
        {
            string selectQuery = "SELECT * FROM new_coach";
